...
class activateOllama(AbstractAction):

    def _start_action(self):
        rospy.loginfo(
            "Obtaining detecting intent from user input: "
            + " ".join(self.params)
            + " ..."
        )
        # self.starting_time = rospy.Time.now()

        self.input = rospy.Publisher("/planner_intention", String, queue_size=1, latch=True)
    ...
    def _stop_action(self):
        ...
        rospy.loginfo("STOPPED activateOllama")
