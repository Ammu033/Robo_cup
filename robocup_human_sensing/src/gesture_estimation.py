...
import joblib
from robocup_human_sensing.msg import RegionOfInterest2D, BodyPosture, Gesture
...
a_yaml_file = open(config_direct+"openpose_config.yaml")
parsed_yaml_file = yaml.load(a_yaml_file, Loader=yaml.FullLoader)
##SKELETON EXTRACTION USING OPENPOSE
selected_features=parsed_yaml_file.get("openpose_config").get("selected_features")
n_joints=parsed_yaml_file.get("openpose_config").get("skeleton_extract_param")
n_features=len([0]*n_joints)+len([0]*(n_joints-2)) # number of features that can be used for gesture recognition, distances+angles
...
openpose_direct=rospy.get_param("/hs_gesture_estimation/openpose_direct") 
openpose_python=openpose_direct+"build/python" 
openpose_models=openpose_direct+"models" 
gesture_classifier_model=rospy.get_param("/hs_gesture_estimation/models_direct") + "classifier_model_3D_v7.joblib"
...
camera_topic_rgb=rospy.get_param("/hs_gesture_estimation/camera_topic_rgb") 
camera_topic_depth=rospy.get_param("/hs_gesture_estimation/camera_topic_depth") 
#########################################################################################################################
...
class human_class:
    def __init__(self): #It is done only the first iteration
        ...
        self.ids=[0] #list with int corresponding to the IDs of the people detected
        ...
        self.roi_confi=np.zeros([self.n_human,1]) #the confidence of a Region of Interest (ROI) 
        ...
        self.new_data=False
        self.image_show=np.zeros((100,100,3), np.uint8) 
        
    def rgbd_callback(self,rgb, depth):
        #rospy.logwarn("CALLBACK 1")
        if self.n_human>0 and self.new_data==True:
            #print("roi",self.roi)
            
            roi=self.roi
            roi_confi=self.roi_confi
            ids=self.ids
            ...
            for i in range(0,n_human):
                ...
                ##############################################
                color_crop = color_image[int(roi[i,1]):int(roi[i,1]+roi[i,3]), int(roi[i,0]):int(roi[i,0]+roi[i,2])].copy() 
                depth_crop = depth_array[int(roi[i,1]):int(roi[i,1]+roi[i,3]), int(roi[i,0]):int(roi[i,0]+roi[i,2])].copy() 
                [gesture[i,:],posture[i,:],orientation[i,:],image_show]=self.processing(color_crop,depth_crop)
                #print("c_max",c_max,"roi",roi_confi[i])
                if c_max<roi_confi[i]:
                    self.image_show=image_show.copy()
                    c_max=roi_confi[i]
            #Publish last OPENPOSE output as an image
            #self.image_show=image_show
            #print("Image",color_crop.shape)
    ...
    def roi_ids_callback(self,data):
        ...
        if self.new_data==False and data.ids!=None:
            n_human=len(data.ids)
            roi=np.zeros([n_human,4],np.int16)
            roi_confi=np.zeros([n_human,1])
            if n_human==0:
                ...
                for i in range(0,n_human):
                    roi[i,0]=int(data.x[i])
                    roi[i,1]=int(data.y[i])
                    roi[i,2]=int(data.w[i])
                    roi[i,3]=int(data.h[i])
                    roi_confi[i]=data.c[i]
                self.new_data=True
                self.roi=roi
                self.roi_confi=roi_confi
                self.ids=data.ids
            self.n_human=n_human
    ...
    def gesture_estimation(self,poseKeypoints,depth_array):
        ...
        if Js!=0 and valid>=joints_min: 
            ...
            features_dist = [dist[i] for i in selected_features]
            features_angles = [angles[i] for i in selected_features]
            features=features_dist+features_angles  #subset of features that are used for the ML model
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('h_boxes_tracked', RegionOfInterest2D,human.roi_ids_callback)
    #rospy.Subscriber('people_roi', RegionOfInterest2D,human.roi_ids_callback)
    image_front_sub = message_filters.Subscriber(camera_topic_rgb, Image) 
    depth_front_sub = message_filters.Subscriber(camera_topic_depth, Image) 
