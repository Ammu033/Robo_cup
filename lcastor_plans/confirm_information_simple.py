...
def confirm_information_simple(p, person, info):
    ...
    # p.exec_action('speak' , '.')
    p.exec_action("activateOllama", "affirm_deny")

    start_time = rospy.get_time()

    # get name from ros params
    detected = p.get_condition("IsOllamaIntentDetected")
    while not detected:
    ...
    try:
        confirmed = rospy.wait_for_message(
            "/ollama_person_affirm_deny", Bool, timeout=10.0
        ).data
