...
def obtain_guest_information(p, person, info):
        ...
        if (rospy.get_time() - start_time) < ollama_timeout:
            ...
            if response and response.success:
                ...
                if response_intent == publish_info:
                    # TODO: process how response_output is obtained and where
                    response_output = rospy.wait_for_message(
    ...
    if response_affirmation == "Yes":
        time.sleep(2)
        rospy.loginfo(f"obtain_person: saving guest data for {person}...")
        p.exec_action(
            "saveGuestDataOllama", f"set{info}_" + person + "_" + response
        )
    else:
        time.sleep(2)
    ...
    if get_name := rospy.get_param(f"/{person}/name"):
        ...
        p.action_cmd(
            "speak", f"Thank_you,_{response}_was_stored_as_your_{info}", "start"
        )
