...
from lcastor_grasping.srv import RelToAbsolute, RelToAbsoluteResponse
from lcastor_grasping.srv import ApproachObject, ApproachObjectResponse
...
def get_absolute_object_pose():
    ...
    try:
        ...
        rel_to_absolute_pose = rospy.ServiceProxy('/lcastor/rel_to_absolute_pose', RelToAbsolute)
     ...
     # in case of lack of communication with server 
        ...
        rospy.logerr("Could not connect to /lcastor/rel_to_absolute_pose service")
        exit()
...
if __name__ == '__main__':
    ...
    sub_target_rel_pose = rospy.Subscriber(
        '/lcastor/target_pose/relative', Pose, get_object_relative_pose)
    ...
    rospy.wait_for_service('/lcastor/rel_to_absolute_pose')
    ...
    rospy.wait_for_service('/lcastor/approach_object')
    try:
        sub_target_rel_pose.unregister()

        approach_object = rospy.ServiceProxy('/lcastor/approach_object', ApproachObject)
        approach_object(object_abs_pose)

    # in case of lack of communication with server 
    except rospy.ServiceException as e:
        rospy.logerr("Could not connect to /lcastor/approach_object service")
        exit()

    # wating for info from service
    rospy.wait_for_service('/lcastor/pick_object')
    try:
        sub_target_rel_pose.unregister()

        pick_object = rospy.ServiceProxy('/lcastor/pick_object', Empty)
        pick_object()
    # in case of lack of communication with server 
    except rospy.ServiceException as e:
        rospy.logerr("Could not connect to /lcastor/pick_object service")
        exit()    