...
def convertCloudFromRosToOpen3d(ros_cloud):
    # Read the point cloud data from the PointCloud2 message
    field_names = [field.name for field in ros_cloud.fields]
    cloud_data = list(pc2.read_points(ros_cloud, skip_nans=True, field_names=field_names))

    # Convert to Open3D point cloud
    xyz = np.array(cloud_data)[:, :3]  # Extract x, y, z coordinates
    o3d_cloud = o3d.geometry.PointCloud()
    o3d_cloud.points = o3d.utility.Vector3dVector(xyz)

    return o3d_cloud

def handle_point_cloud(pcl):
    # Convert ROS PointCloud2 message to Open3D point cloud
    try:
        o3d_cloud = convertCloudFromRosToOpen3d(pcl)
        print("Converted Open3D point cloud: ", o3d_cloud)
    except Exception as e:
        rospy.logerr("Error converting PointCloud2 to Open3D point cloud: %s", e)
        return None

    # Extract numpy array from Open3D point cloud
    pcl_np = np.asarray(o3d_cloud.points)
...
def generate_grasps(req):
    ...
    print("req.full_pcl",type(req.full_pcl))
    #
    #print("pcl: ",pcl)
    #
    pcl_np = handle_point_cloud(req.full_pcl)
    print("pcl_np: ",pcl_np)
    print("pcl_np shape handle point clouds: ",pcl_np.shape)

    #pcl = ros_numpy.numpify(req.full_pcl)
    #pcl_np = np.concatenate( (pcl['x'].reshape(-1,1), pcl['y'].reshape(-1,1), pcl['z'].reshape(-1,1)), axis=1)
    #print("pcl_np shape: ",pcl_np.shape)
    ...
    for i, object_pcl in enumerate(req.objects_pcl):
        ...
        #objs_np[i] = obj_np
        objs_np[i] = handle_point_cloud(object_pcl)
        print("objs_np shape: ",objs_np[i].shape)
    ...
    # 
    print("------------------------>",pred_grasps_cam, scores, contacts, openings)
