...
MODEL_PATH = os.path.join(rospkg.RosPack().get_path('contact_graspnet'), 'src/contact_graspnet/utils/maskrcnn_newest.pth')
print(MODEL_PATH, rospkg.RosPack().get_path('contact_graspnet'))
LABEL_PATH = os.path.join(rospkg.RosPack().get_path('contact_graspnet'), 'src/contact_graspnet/utils/labels.txt')
YCB_LABELS_PARTIAL = read_file_to_list(LABEL_PATH)
mask_rcnn = YcbMaskRCNN(MODEL_PATH, YCB_LABELS_PARTIAL)
...
def detect(msg):
    ...
    #pclmsg = rospy.wait_for_message('/zed2i/zed_node/point_cloud/cloud_registered', PointCloud2)
